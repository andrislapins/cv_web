...
resource "hcloud_load_balancer_service" "istio_gateway_lb_http" {
  ...
  listen_port      = 80
  destination_port = "33333" # local.istio_gateway_http_nodeport
  ...
  health_check {
    ...
    protocol = "tcp"
    port     = "33333" # local.istio_gateway_http_nodeport
  }
}
...
resource "hcloud_load_balancer_service" "istio_gateway_lb_https" {
  ...
  listen_port      = 443
  destination_port = "33334" # local.istio_gateway_https_nodeport
  ...
  health_check {
    ...
    protocol = "tcp"
    port     = "33334" # local.istio_gateway_https_nodeport
  }
}